...
export function ChatInterface({
  ...
  return <div className="flex flex-col h-full bg-gradient-to-br from-gray-50 via-gray-100 to-gray-200 dark:from-gray-900 dark:via-gray-800 dark:to-gray-700 relative overflow-hidden">
      ...
      <div className="sticky top-0 z-20 p-6 border-b border-gray-400 dark:border-gray-500 bg-gray-100/70 dark:bg-gray-800/70 backdrop-blur-2xl backdrop-saturate-150 transition-all duration-300">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-3">
              {backendStatus === 'checking' && <div className="flex items-center gap-2 animate-fade-in">
                  ...
                  <Badge variant="outline" className="text-amber-700 border-amber-400 bg-amber-50/80 dark:bg-amber-950/30 dark:border-amber-600 backdrop-blur-sm transition-all duration-300">
                    Connecting...
                  </Badge>
                </div>}
              {backendStatus === 'ready' && <div className="flex items-center gap-2 animate-fade-in">
                  ...
                  <Badge variant="outline" className="text-emerald-700 border-emerald-400 bg-emerald-50/80 dark:bg-emerald-950/30 dark:border-emerald-600 backdrop-blur-sm transition-all duration-300">
                    Ready
                  </Badge>
                </div>}
              {backendStatus === 'error' && <div className="flex items-center gap-2 animate-fade-in">
                  <div className="w-3 h-3 bg-red-500 rounded-full animate-pulse"></div>
                  <Badge variant="outline" className="text-red-700 border-red-400 bg-red-50/80 dark:bg-red-950/30 dark:border-red-600 backdrop-blur-sm transition-all duration-300">
                    Disconnected
                  </Badge>
                </div>}
            </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 pb-32 relative">
        ...
        {(currentQuestion || currentAnswer || isLoading) && <div className="max-w-4xl mx-auto space-y-8">
            ...
            {currentQuestion && <div className="flex justify-end animate-slide-in-right">
                <div className="bg-gradient-to-r from-gray-700 to-gray-800 dark:from-gray-600 dark:to-gray-700 text-white rounded-3xl rounded-br-lg px-8 py-6 max-w-2xl shadow-2xl shadow-gray-500/25 border border-gray-600 dark:border-gray-500 transform transition-all duration-300 hover:scale-[1.02] hover:shadow-3xl">
                  <p className="text-sm leading-relaxed font-medium">{currentQuestion}</p>
                </div>
              </div>}

            {/* Assistant response with enhanced design and speaker button */}
            {(currentAnswer || isLoading) && <div className="flex justify-start animate-slide-in-left">
                <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-xl border border-gray-400 dark:border-gray-500 rounded-3xl rounded-bl-lg px-8 py-6 max-w-3xl shadow-2xl shadow-gray-500/10 transform transition-all duration-300 hover:scale-[1.01] hover:shadow-3xl relative">
                  {/* Speaker Button */}
                  {currentAnswer && !isLoading && (
                    <Button
                      ...
                      variant="ghost"
                      size="icon"
                      className="absolute top-4 right-4 h-8 w-8 rounded-full bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 border border-gray-400 dark:border-gray-500 transition-all duration-300 hover:scale-110"
                    >
                      {isSpeaking ? (
                  ...
                  {isLoading ? <div className="flex items-center gap-4 py-2">
                    ...
                    </div> : <div className="prose prose-sm max-w-none pr-12">
            ...
            {showFeedback && currentAnswer && <div className="flex justify-start animate-fade-in delay-150">
                <div className="max-w-md">
                  <Card className="p-6 border-gray-400 dark:border-gray-500 shadow-2xl bg-white/80 dark:bg-gray-800/80 backdrop-blur-2xl backdrop-saturate-150 transform transition-all duration-500 hover:scale-[1.02] hover:shadow-3xl">
                    ...
                    <div className="flex gap-3 mb-5">
                      <Button variant={feedbackRating === '👍 Yes' ? 'default' : 'outline'} size="sm" onClick={() => setFeedbackRating('👍 Yes')} className="flex-1 rounded-2xl transition-all duration-300 hover:scale-105 hover:shadow-lg border-gray-400 dark:border-gray-500">
                        <ThumbsUp className="h-4 w-4 mr-2 transition-transform duration-300 group-hover:scale-110" />
                        Yes
                      </Button>
                      <Button variant={feedbackRating === '👎 No' ? 'default' : 'outline'} size="sm" onClick={() => setFeedbackRating('👎 No')} className="flex-1 rounded-2xl transition-all duration-300 hover:scale-105 hover:shadow-lg border-gray-400 dark:border-gray-500">
                        <ThumbsDown className="h-4 w-4 mr-2 transition-transform duration-300 group-hover:scale-110" />
                        No
                      </Button>
                    </div>
                    <Textarea placeholder="Share your thoughts (optional)..." value={feedbackComment} onChange={e => setFeedbackComment(e.target.value)} className="mb-5 rounded-2xl border-gray-400 dark:border-gray-500 resize-none transition-all duration-300 focus:ring-2 focus:ring-gray-500/20 focus:border-gray-500" rows={3} />
