      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8 animate-fade-in">
          <div className="bg-white/90 backdrop-blur-xl border-0 shadow-2xl rounded-3xl p-8 mb-6 transition-all duration-500 hover:shadow-3xl hover:scale-[1.02]">
            ...
            <p className="text-gray-600 text-lg font-medium">Ask Questions about Bignlaytics Coaching</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-1 animate-slide-in-left">
            <Card className="bg-white/95 backdrop-blur-xl border-2 border-blue-200/50 shadow-2xl rounded-3xl overflow-hidden transition-all duration-500 hover:shadow-3xl hover:scale-[1.02] hover:border-blue-300/70">
              ...
              <CardContent className="space-y-4 max-h-[600px] overflow-y-auto p-6 custom-scrollbar">
                {chatHistory.length > 0 ? chatHistory.map((chat, index) => <Card key={index} className="p-4 bg-gradient-to-br from-gray-50 to-blue-50 border-2 border-blue-100 shadow-lg rounded-2xl transition-all duration-300 hover:shadow-xl hover:scale-[1.02] hover:border-blue-200 animate-fade-in" style={{
                animationDelay: `${index * 100}ms`
                    ...
                    <div className="space-y-3">
                      ...
                      {chat.feedback && <div className="text-xs text-gray-500 bg-white/80 rounded-lg p-3 border-2 border-gray-200 shadow-sm">
          ...
          <div className="lg:col-span-3 space-y-8 animate-slide-in-right">
              ...
              <CardContent className="p-8">
                <form onSubmit={handleSubmitQuestion} className="space-y-6">
                  <div className="relative">
                    <Input placeholder="Ask a question about knowledge.pdf..." value={question} onChange={e => setQuestion(e.target.value)} className="text-lg p-6 rounded-2xl border-2 border-gray-300 focus:border-blue-400 transition-all duration-300 focus:shadow-lg focus:scale-[1.02] bg-gray-50/50 shadow-inner" disabled={isLoading || backendStatus !== 'ready'} />
                    <div className="absolute inset-y-0 right-4 flex items-center pointer-events-none">
                      <FileText className="w-6 h-6 text-gray-400" />
                    </div>
                  </div>
                  <Button type="submit" className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white py-6 text-lg font-bold rounded-2xl transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl disabled:opacity-50 disabled:scale-100 shadow-lg" disabled={isLoading || !question.trim() || backendStatus !== 'ready'}>
                    {isLoading ? <>
            ...
            {/* Answer Display */}
            {currentAnswer && <Card className="bg-white/95 backdrop-blur-xl border-2 border-green-200/50 shadow-2xl rounded-3xl overflow-hidden transition-all duration-500 hover:shadow-3xl hover:border-green-300/70 animate-fade-in">
            ...
            {/* Feedback Form */}
            {showFeedback && currentAnswer && <Card className="bg-white/95 backdrop-blur-xl border-2 border-purple-200/50 shadow-2xl rounded-3xl overflow-hidden transition-all duration-500 hover:shadow-3xl hover:border-purple-300/70 animate-fade-in">
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <div className="flex gap-4">
                        <Button type="button" variant={feedbackRating === '👍 Yes' ? 'default' : 'outline'} onClick={() => setFeedbackRating('👍 Yes')} className={`flex items-center gap-3 px-6 py-4 rounded-2xl transition-all duration-300 hover:scale-105 font-semibold ${feedbackRating === '👍 Yes' ? 'bg-gradient-to-r from-green-500 to-emerald-500 text-white shadow-lg border-2 border-green-300' : 'hover:bg-green-50 border-2 border-green-300'}`}>
                          <ThumbsUp className="w-5 h-5" />
                          👍 Yes
                        </Button>
                        <Button type="button" variant={feedbackRating === '👎 No' ? 'default' : 'outline'} onClick={() => setFeedbackRating('👎 No')} className={`flex items-center gap-3 px-6 py-4 rounded-2xl transition-all duration-300 hover:scale-105 font-semibold ${feedbackRating === '👎 No' ? 'bg-gradient-to-r from-red-500 to-pink-500 text-white shadow-lg border-2 border-red-300' : 'hover:bg-red-50 border-2 border-red-300'}`}>
                    ...
                    <div>
                      <Textarea placeholder="Any comments or suggestions?" value={feedbackComment} onChange={e => setFeedbackComment(e.target.value)} rows={4} className="rounded-2xl border-2 border-gray-300 focus:border-purple-400 transition-all duration-300 focus:shadow-lg bg-gray-50/50 text-lg p-4 shadow-inner" />
                    </div>
                    <Button onClick={handleSubmitFeedback} className="w-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white py-6 text-lg font-bold rounded-2xl transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl shadow-lg" disabled={!feedbackRating || isSubmittingFeedback}>
